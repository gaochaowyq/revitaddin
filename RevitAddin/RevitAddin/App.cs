...
namespace RevitAddin
{
    ...
    class App : IExternalApplication
    {
        ...
        public Result OnStartup(UIControlledApplication a)
        {
            ...
            if (panel.AddItem(
                new PushButtonData("WPF Template", "WPF Template", thisAssemblyPath,
                    "RevitAddin.EntryCommand")) is PushButton button)
            {
                ...
                Uri uriImage = new Uri("pack://application:,,,/RevitAddin;component/Resources/code-small.png");
                BitmapImage largeImage = new BitmapImage(uriImage);
                button.LargeImage = largeImage;
            }

            if (panel.AddItem(new PushButtonData("ImportRhinoBlock", "IRB", thisAssemblyPath,"RevitAddin.IRBCommand")) is PushButton button1)
            {
                // defines the tooltip displayed when the button is hovered over in Revit's ribbon
                button1.ToolTip = "Import Rhino Block to Revit With Family";
                // defines the icon for the button in Revit's ribbon - note the string formatting
                Uri uriImage = new Uri("pack://application:,,,/RevitAddin;component/Resources/code-small.png");
                BitmapImage largeImage = new BitmapImage(uriImage);
            ...
            if (panel.AddItem(
                new PushButtonData("WPF Template\nMulti-Thread", "WPF Template\nMulti-Thread", thisAssemblyPath,
                    "RevitAddin.EntryCommandSeparateThread")) is PushButton button2)
            {
        ...
        {
            string tab = "RhinoToRevit"; // Tab name
            // Empty ribbon panel 
            ...
            try
            {
                RibbonPanel panel = a.CreateRibbonPanel(tab, "RhinoBlock");
            }
            catch (Exception ex)
            {
                Util.HandleError(ex);
            ...
            foreach (RibbonPanel p in panels.Where(p => p.Name == "RhinoBlock"))
            {
                ribbonPanel = p;
            }
